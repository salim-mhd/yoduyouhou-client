...
const AdminNavbar = () => {
  ...
  return (
    <>
      <header className="shadow bg-blue-600 top-0 mb-2">
        <div className="relative flex max-w-screen-xl flex-col overflow-hidden px-4 py-4 md:mx-auto md:flex-row md:items-center">
          <a href="#" className="flex items-center whitespace-nowrap text-2xl font-black">
            ...
            <span className="text-black">Welcome {localStorage.getItem('admin') ? <span>Admin</span> : <></>}</span>
          </a>
          ...
          <nav aria-label="Header Navigation" className="peer-checked:mt-8 peer-checked:max-h-56 flex max-h-0 w-full flex-col items-center justify-between overflow-hidden transition-all md:ml-24 md:max-h-full md:flex-row md:items-start">
            <ul className="flex flex-col items-center space-y-2 md:ml-auto md:flex-row md:space-y-0">
              <li className="text-gray-600 md:mr-12 hover:text-white">
                {localStorage.getItem('admin', true) ? <button onClick={(e) => { logout() }} className="rounded-md border-2 border-white px-6 py-1 font-medium text-white transition-colors hover:bg-red-600 hover:text-white">Log out</button> : <></>}
              </li>
            </ul>
          </nav>
        </div>
